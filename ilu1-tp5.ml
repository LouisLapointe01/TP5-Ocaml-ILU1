let rec insertObj o liste =
  match liste with 
  |[] -> (1,o):: liste |(x,y):: list-> if y=o then ((x+1),y)::list else(x,y):: insertObj o list;;
...
let rec aumoins_indexed n liste = 
  match liste with
  |[]-> []
  |(x,y):: list -> if (longueur y) >= n then x :: aumoins_indexed n liste
      else aumoins_indexed n list;;